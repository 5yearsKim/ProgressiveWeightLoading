...
def convert_hf_to_block_resnet(hf_sd: StateDictT) -> StateDictT:
    ...
    # 1) Stem (embedder)
    blk_sd["resnet.blocks.0.0.embedder.convolution.weight"] = \
        hf_sd["resnet.embedder.embedder.convolution.weight"]
    for p in ("weight", "bias", "running_mean", "running_var", "num_batches_tracked"):
        blk_sd[f"resnet.blocks.0.0.embedder.normalization.{p}"] = \
            hf_sd[f"resnet.embedder.embedder.normalization.{p}"]
    ...
    for i in (0, 1):
        for j in (0, 1):
            ...
            # conv weight
            blk_sd[f"{blk_pref}.convolution.weight"] = hf_sd[f"{ms_pref}.convolution.weight"]
            # batch‐norm params
            for p in ("weight", "bias", "running_mean", "running_var", "num_batches_tracked"):
                blk_sd[f"{blk_pref}.normalization.{p}"] = hf_sd[f"{ms_pref}.normalization.{p}"]
    ...
    for n in (1, 2, 3):
        for i in (0, 1):
            ...
            if i == 0:
                blk_sd[f"{blk_layer}.shortcut.convolution.weight"] = \
                    hf_sd[f"{ms_layer}.shortcut.convolution.weight"]
                for p in ("weight", "bias", "running_mean", "running_var", "num_batches_tracked"):
                    blk_sd[f"{blk_layer}.shortcut.normalization.{p}"] = \
                        hf_sd[f"{ms_layer}.shortcut.normalization.{p}"]
